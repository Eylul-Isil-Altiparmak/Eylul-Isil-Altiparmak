...
# Atama Operatoru H2 P:33/110
x <- 1
x <- x+1

# SOR: H2 P:34/110'da harf_listesi = [a,b,c,d,e,f] ve harf_listesi[3:5] neden olmadi?

# Us Operatoru H2 P:35/100
3^2
...
#Veri Tiplerini Cevirme Fonksiyonlari H2 P:73/110
kullanici_no <- 1234
...
my_function<-function(x,y,z){
  x,y ve z ile yapilacak operasyonlar
}
...
# Tanimlanan degiskenler calisma alani icinde saklanmaz. H3 P:52/83
avg<-function(x){
  s<-sum(x)
  n<-length(x)
  s/n
...
for(i in deger aldigi araligi){
  i'nin deger araligi boyuca i'yi kullanan operasyonlar
}
for(i in 1:10){
  print(i)
}

# Ardasik Sayilarin Toplamı H3 P:58/83
